...
async function getExcel() {
  ...
  if(goodsData.length != 0) {
    ...
    ws1.columns = [
      {key: 'goodsNo', width: 15}, {key: 'brandCd', width: 15}, {key: 'purchaseNm', width: 15}, {key: 'goodsNm', width: 30},
    ...
    ws2.columns = [
      {key: 'goodsNo', width: 15}, {key: 'brandCd', width: 15}, {key: 'purchaseNm', width: 15}, {key: 'goodsNm', width: 30},
      {key: 'sno', width: 10}, {key: 'optionViewFl', width: 10},
      {key: 'optionSellFl', width: 10}, {key: 'optionSellFl', width: 10}, {key: 'optionCode', width: 50}, {key: 'optionDeliveryFl', width: 10}
    ];
...
async function getProduct(pageNo) {
  ...
  if(selectedGoodsData.length == 0) {
    ...
    goodsDataArray = selectedGoodsData.map( r => 
      [ r.goodsNo[0], r.brandCd[0], r.purchaseNm == undefined ? null : r.purchaseNm[0], r.goodsNm[0],
        r.goodsDisplayFl[0],r.goodsSellFl[0],r.goodsCd[0], r.goodsReserveOrderMessage[0] ]
    );
  }
  ...
  if(selectedOptionData.length == 0) {
    ...
    for(let i = 0; i < selectedOptionData.length; i++) {
      ...
      let temp = [ r.goodsNo[0], r.brandCd, r.purchaseNm, r.goodsNm, r.sno[0], r.optionViewFl[0],r.optionSellFl[0],r.optionCode[0], r.optionDeliveryFl[0] ];
